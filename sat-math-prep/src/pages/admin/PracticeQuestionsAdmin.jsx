...
export default function PracticeQuestionsAdmin() {
  ...
  const filtered = questions.filter((q) => {
    ...
    // Exclusively show standard practice questions on this dashboard
    return matchesTopic && currentUsage === "practice";
  });

  const save = async (f) => {
    // Force EVERYTHING on this page to save as a standard practice question
    const payload = { 
      ...f, 
  ...
  return (
    <div className="p-6 md:p-10 max-w-5xl mx-auto" onPaste={handlePaste}>
      <div className="mb-5">
        ...
        <p className="text-gray-500 text-sm">Manage the master practice question repository for topical drills.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-10 text-center text-gray-400 text-sm">
          No practice questions match these topic filters.
        </div>
      ) : (
